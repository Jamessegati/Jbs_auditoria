...
function render() {
  ...
  const feedback = Number.isInteger(answers[question.id]) ? `<aside class="feedback"><b>${answers[question.id] === 0 ? 'Resposta correta.' : `Gabarito: ${esc(question.options[0])}`}</b><p>${esc(question.explanation)}</p></aside>` : '';
